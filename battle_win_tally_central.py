...
COUNTRY_CATEGORIES = {
    'France': [
        'Category:Battles_involving_France'
    ],
    'Germany': [
        'Category:Battles_involving_Germany'
    ],
    'Poland': [
        'Category:Battles_involving_Poland'
    ],
    'Turkey': [
        'Category:Battles_involving_Turkey'
    ],
    'Spain': [
        'Category:Battles_involving_Spain'
    ]
}
...
def main():
    ...
    try:
        df.to_csv('battle_results_raw_expanded.csv', index=False)
        print("Raw results written to battle_results_raw_expanded.csv")
    except Exception as e:
        print("Error writing raw results:", e)

    try:
        win_counts = df[df['result'] == 'win'].groupby('country').size().reset_index(name='wins')
        win_counts.to_csv('battle_win_counts_expanded.csv', index=False)
        print("Win counts written to battle_win_counts_expanded.csv")
        print("\nWin counts:")
        print(win_counts)
    except Exception as e:
